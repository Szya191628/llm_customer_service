...
@click.command("chat", help="启动用户交互页面")
@click.option(
    "--model", "-m",
...
def chat_command(
    ...
    示例:
        atguigu chat
        atguigu chat --model ./my_bot --port 5005
        atguigu chat --no-browser
    """
    ...
    try:
        ...
        from app.api.server import AtguiguServer
        ...
        browser_url = f"http://{browser_host}:{port}/chat"
        
        click.echo()
        click.echo(f"Chat页面: {chat_url}")
