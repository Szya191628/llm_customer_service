...
from app.retrieval.base_retriever import Retriever, Document, SearchResult
from app.retrieval.embedder import Embedder, create_embedder

if TYPE_CHECKING:
    from app.dialogue_understanding.flow import Flow, FlowsList
...
class FlowRetriever(Retriever):
    ...
    async def index_flows(self, flows: List["Flow"]) -> None:
        ...
        for doc, emb in zip(self._flow_documents, embeddings):
            doc.embedding = emb
        
        self._index_built = True
        logger.info(f"Indexed {len(self._flow_documents)} flows (batch)")
