...
from app.dialogue_understanding.generator.base_generator import (
    CommandGenerator,
    GeneratorConfig,
    GenerationResult,
)
from app.dialogue_understanding.generator.prompt_builder import PromptBuilder
from app.dialogue_understanding.generator.command_parser import (
    CommandParser,
    ParseResult,
)
from app.dialogue_understanding.commands.answer_commands import CannotHandleCommand
from app.dialogue_understanding.commands.error_commands import (
    ErrorCommand,
...
class LLMCommandGenerator(CommandGenerator):
    ...
    async def generate(
        ...
        try:
            ...
            print(f"[DEBUG] LLM response: {repr(llm_response.content)}")
            logger.info(f"LLM response: {repr(llm_response.content)}")
            ...
            if parse_result.errors:
                ...
                print(f"[DEBUG] Parse errors: {parse_result.errors}")

            # 4. 如果没有解析出命令，返回cannot_handle
            if not result.commands:
                print(f"[DEBUG] No commands parsed from: {repr(llm_response.content)}")
