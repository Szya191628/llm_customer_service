...
from app.shared.constants import DegradationReason
...
class CannotHandleCommand(Command):
    ...
    def regex_pattern(cls) -> str:
        """返回正则表达式模式。"""
        return r"""^(?:cannot_handle(?:\(['"]?(.*)['"]?\))?|CannotHandle\((?:['"]?(.*)['"]?)?\)|无法处理)$"""
...
class KnowledgeAnswerCommand(Command):
    ...
    def regex_pattern(cls) -> str:
        """返回正则表达式模式。"""
        return r"""^(?:knowledge_answer(?:\(['"]?(.*)['"]?\))?|SearchAndReply\((?:['"]?(.*)['"]?)?\)|知识回答|知识查询)$"""
...
class FAQCommand(Command):
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "FAQCommand":
        """从字典创建命令。"""
        return FAQCommand()
    ...
    def run(
        ...
        注意：这里只是触发FAQ action，实际执行由框架完成。
        """
        # 压入一个标记，让框架知道需要执行FAQ action
        return [{
            "event": "faq_triggered",
            "timestamp": None,
        }]
    ...
    @classmethod
    def _from_regex_match(cls, match: re.Match) -> Optional["FAQCommand"]:
        """从正则匹配创建命令。"""
        return FAQCommand()
