...
class GraphRAG(InformationRetrieval):
    ...
    def __init__(self, embeddings=None):
        ...
        self.route_label_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessagePromptTemplate.from_template(
                    ...
                    '以严格JSON数组格式输出，不要输出任何其他文字。\n'
                    "示例:\n"
                    '用户输入: "iPhone 16 Pro的价格是多少"\n'
                    '输出: [{{"label":"SPU","entity":"iPhone 16 Pro"}}]\n'
                    '用户输入: "帮我查一下用户张三的订单"\n'
                    '输出: [{{"label":"User","entity":"张三"}},{{"label":"Order","entity":"张三"}}]\n'
                    '用户输入: "手机和耳机哪个更好"\n'
                    '输出: [{{"label":"SPU","entity":"手机"}},{{"label":"SPU","entity":"耳机"}}]\n'
                    "可选节点类型:\n{optional_label}"
                ),
        ...
        self.generate_cypher_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessagePromptTemplate.from_template(
                    "你是一个Cypher专家，正在根据入口节点信息和用户输入，参照schema生成Cypher查询语句。\n"
                    "**注意：查询结果中不可以包含嵌入向量等多余属性**\n"
                    "仅返回Cypher语句，禁止任何解释、注释、markdown标记、前缀文字。\n"
                    "示例:\n"
                    "入口节点: {{'SPU': ['iPhone 16 Pro']}}\n"
                    "用户输入: iPhone 16 Pro的价格是多少\n"
                    "Cypher语句:\n"
                    "MATCH (s:SPU) WHERE s.name = 'iPhone 16 Pro' RETURN s.price, s.name\n"
                    "---\n"
                    "入口节点: {{'SPU': ['iPhone 16 Pro'], 'SKU': ['256GB']}}\n"
                    "用户输入: iPhone 16 Pro 256GB版本有哪些颜色\n"
                    "Cypher语句:\n"
                    "MATCH (s:SPU)-[:HAS_SKU]->(sku:SKU) WHERE s.name = 'iPhone 16 Pro' AND sku.name CONTAINS '256GB' RETURN sku.name, sku.color\n"
                    "---\n"
                    "schema:\n{schema}"
                ),
        ...
        self.correct_cypher_prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessagePromptTemplate.from_template(
                    ...
                    "仅返回Cypher语句。\n"
                    "**严格规则：**\n"
                    "1. 只输出Cypher语句本身，禁止任何解释、注释、markdown标记\n"
                    "2. 不要写```cypher```代码块标记\n"
                    "3. 不要写\"以下是...\"等前缀文字\n"
                    "4. 第一个字符必须是 M 或 W（MATCH/WITH/WHERE）\n"
                    "schema:\n{schema}"
                ),
                HumanMessagePromptTemplate.from_template(
                    "入口节点:\n{entry_nodes}\n\n"
    ...
    def connect(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        model_api_base = os.getenv("LLM_API_BASE", "https://token-plan-sgp.xiaomimimo.com/v1")
        model_name = os.getenv("LLM_MODEL", "mimo-v2.5")
        model_temperature = float(os.getenv("LLM_TEMPERATURE", "0.1"))
        self.llm = ChatOpenAI(
            model=model_name,
    ...
    async def route_label(self, query):
        ...
        # 2、调用LLM，获得输出结果（最多2次尝试：正常prompt + 简化prompt）
        outputs = []
        for attempt in range(2):
            try:
                if attempt == 0:
                    ...
                    # 第2次：用极简prompt重试
                    logger.warning("路由解析第1次失败，用简化prompt重试...")
                    simple_prompt = self.route_label_prompt.format_prompt(
                        optional_label=self.optional_label, query=query
                    )
                    llm_output = await self.llm.ainvoke(simple_prompt)

                raw = llm_output.content.strip()

                # 策略A: 原有的_extract_json_from_text方法
                json_str = self._extract_json_from_text(raw)

                # 策略B: 如果策略A返回空数组，用正则直接匹配label/entity对
                if json_str == "[]":
                    import re
                    pairs = re.findall(
                        r'"label"\s*:\s*"([^"]+)".*?"entity"\s*:\s*"([^"]+)"',
                        raw, re.DOTALL
                    )
                    if pairs:
                        json_str = json.dumps([{"label": l, "entity": e} for l, e in pairs], ensure_ascii=False)

                # 策略C: 如果还没有结果，尝试匹配有序列表格式如 "1. 标签：SPU 实体：手机"
                if json_str == "[]":
                    list_pairs = re.findall(
                        r'(?:标签|label)[：:]\s*(\w+)\s*(?:实体|entity)[：:]\s*(\S+)',
                        raw
                    )
                    if list_pairs:
                        json_str = json.dumps([{"label": l, "entity": e} for l, e in list_pairs], ensure_ascii=False)

                outputs = [RouteItem(**item) for item in json.loads(json_str)]
                if outputs:  # 成功提取，跳出重试循环
                    break

            except Exception as e:
                logger.warning(f"路由解析第{attempt+1}次失败: {e}，LLM输出: {llm_output.content[:200] if 'llm_output' in dir() else 'N/A'}")
                if attempt == 1:
                    logger.error("路由解析两次尝试均失败，返回空结果")
                    outputs = []
    ...
    async def generate_cypher(self, query, entry_nodes):
        ...
        for attempt in range(2):
            ...
            # 4、extract_cypher提取失败时，多策略正则兜底
            if not cypher:
                import re
                # 策略A: 匹配 MATCH/MERGE/WITH/UNWIND 开头的语句
                match = re.search(r'((?:MATCH|MERGE|WITH|UNWIND)[\s\S]*?)(?:```|$)', raw, re.IGNORECASE)
                if match:
                    cypher = match.group(1).strip().rstrip(';')
                else:
                    # 策略B: 取第一行以大写字母开头的非空行（跳过解释文字）
                    for line in raw.split('\n'):
                        line = line.strip()
                        if line and re.match(r'^[A-Z]', line) and not line.startswith(('以下是', '根据', '注意', '说明', '这里', '#', '//')):
                            cypher = line
                            break
                if cypher:
                    logger.info("Cypher生成(兜底提取,第%d次):%s", attempt+1, cypher)
                else:
                    logger.warning("Cypher第%d次生成失败，LLM输出:%s", attempt+1, raw[:200])

            if cypher:  # 成功提取，跳出重试循环
                break
            logger.warning("Cypher提取为空，第%d次重试...", attempt+1)
    ...
    async def search(
        ...
        执行查询（优化版）
        优化策略：
          方案A - 跳过validate_cypher，直接执行Cypher；仅在执行失败时调用correct_cypher校正
          方案B - route_label 与 embedding 预计算并行执行
        预期LLM调用从4-5次减少到2-3次（正常路径），耗时从85s降至40-50s
        """
        ...
        # ========== 方案B: route_label 与 embedding 预计算并行 ==========
        # embedding 只依赖 query 文本，不依赖 route 结果，可并行
        route_task = self.route_label(chat_history)
        embed_task = asyncio.to_thread(self.embeddings.embed_query, chat_history)

        route_res, _ = await asyncio.gather(route_task, embed_task)
        # embedding 结果暂存，node_retrieval 内部会重新计算（此处预热模型+缓存）

        # 检索入口节点
        entry_nodes = await self.node_retrieval(route_res, 10)

        # 生成 Cypher 语句（LLM调用 #1）
        cypher = await self.generate_cypher(query, entry_nodes)

        # 校正关系方向（非LLM，LangChain内置规则校正）
        cypher = self.cypher_corrector(cypher)
        logger.info("Cypher校正(关系方向):%s", cypher)

        # ========== 方案A: 跳过validate，直接执行Cypher ==========
        # 正常路径: generate_cypher → cypher_corrector → execute（省去1次LLM调用）
        # 失败路径: execute异常 → correct_cypher(LLM) → cypher_corrector → 重试execute
        results = []
        try:
            ...
            logger.warning("Cypher执行失败，尝试LLM校正: %s", e)
            # 执行失败时才调用LLM校正（方案A核心：延迟LLM调用到真正需要时）
            try:
                cypher = await self.correct_cypher(query, entry_nodes, cypher, [str(e)])
                cypher = self.cypher_corrector(cypher)
                logger.info("校正后Cypher:%s", cypher)
                records = self.driver.execute_query(cypher).records
                for rec in records:
                    record_dict = dict(rec)
                    text = self._format_record_as_text(record_dict)
                ...
                logger.error("校正后Cypher仍执行失败: %s", e2)
